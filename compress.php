		...
		if (is_dir($compress_entry) === true) {
				...
				foreach ($files as $file) {
					...
					if (is_dir($file) === true) {
						...
						$zip->addFile($file,str_replace($compress_entry . DIRECTORY_SEPARATOR, '', $file));

					}
				}
			}
			elseif (is_file($compress_entry) == true) {
				$zip->addFile($compress_entry,basename($compress_entry));
			}
